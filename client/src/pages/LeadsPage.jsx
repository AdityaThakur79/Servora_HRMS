...
const emptyForm = {
    ...
    source: 'manual', status: 'new-lead', assignedTo: '', salesOwner: '', description: '',
};
...
function GroupedStatusSelect({ value, onChange, style }) {
    const normalized = normalizeLegacyStatus(value);
    return (
        <select className="form-select" value={normalized} onChange={onChange} style={style}>
...
export default function LeadsPage() {
    ...
    const fetchLeads = async () => {
        ...
        try {
            const res = await api.get('/leads', {
                params: { search, status: statusFilter || undefined, assignedTo: assignedToFilter || undefined, page, limit },
            });
    ...
    useEffect(() => {
        Promise.all([fetchStats(), fetchUsers(), fetchLeads()]).catch(() => toast.error('Unable to load leads'));
    }, []); // eslint-disable-line

    useEffect(() => { fetchLeads().catch(() => {}); }, [page, statusFilter, assignedToFilter]); // eslint-disable-line

    const onSearchSubmit = (e) => { e.preventDefault(); setPage(1); fetchLeads().catch(() => {}); };
    ...
    const openEdit = (lead) => {
        ...
        setForm({
            ...
            status: normalizeLegacyStatus(lead.status || 'new-lead'),
            assignedTo: lead.assignedTo?._id || '',
            salesOwner: lead.salesOwner?._id || '',
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (!payload.salesOwner) delete payload.salesOwner;
            if (editing) { await api.put(`/leads/${editing._id}`, payload); toast.success('Lead updated'); }
    ...
    const handleDelete = async (id) => {
        ...
        try { await api.delete(`/leads/${id}`); toast.success('Lead deleted'); await Promise.all([fetchStats(), fetchLeads()]); }
        catch (err) { toast.error(err.response?.data?.message || 'Not allowed'); }
    };
    ...
    const conversionText = useMemo(() => (!stats ? '—' : `${stats.conversionRate}%`), [stats]);

    return (
        <div className="fade-in">
            <div className="page-header">
                <div><h2>Leads</h2><p>Track, convert and manage your pipeline</p></div>
                <button onClick={openCreate} className="btn btn-primary"><HiOutlinePlus /> New Lead</button>
            </div>
            ...
            <div className="card">
                <div className="card-header">
                    <form onSubmit={onSearchSubmit} style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
                        <div className="search-input-wrapper">
                            <HiOutlineSearch className="search-icon" />
                            <input className="form-input" placeholder="Search name/company/email/phone..." value={search} onChange={(e) => setSearch(e.target.value)} style={{ width: 260 }} />
                        </div>
                        <select className="form-select" value={statusFilter} onChange={(e) => { setStatusFilter(e.target.value); setPage(1); }} style={{ width: 200 }}>
                            ...
                            {LEAD_PIPELINE_GROUPS.map(group => (
                                <optgroup key={group.label} label={group.label}>
                                    {group.statuses.map(s => <option key={s} value={s}>{getStatusInfo(s).label}</option>)}
                                </optgroup>
                            ))}
                        </select>
                        <select className="form-select" value={assignedToFilter} onChange={(e) => { setAssignedToFilter(e.target.value); setPage(1); }} style={{ width: 180 }}>
                    ...
                    <table>
                        ...
                        <tbody>
                            {loading ? (
                                ...
                                <tr><td colSpan={8}><div className="empty-state"><div className="empty-icon">📈</div><h3>No leads found</h3><p>Create a lead to start tracking conversions</p></div></td></tr>
                            ) : rows.map((lead) => {
                                    ...
                                    <tr key={lead._id}>
                                        ...
                                        <td>
                                            <GroupedStatusSelect
                                                ...
                                                style={{ width: 190, minHeight: 34, padding: '6px 10px', fontSize: 12 }}
                                            />
                                        </td>
                                        <td>
                                            {lead.assignedTo ? (
                                                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                                                    <div className="user-avatar" style={{ width: 24, height: 24, fontSize: 10 }}>{lead.assignedTo.name[0]}</div>
                                        ...
                                        <td style={{ maxWidth: 200, position: 'relative' }}>
                                            {lastNote ? (
                                                <span className="note-cell-truncated" style={{ fontSize: 13, color: 'var(--text-secondary)', cursor: 'default', display: 'inline-block', position: 'relative' }}>
                                                    {truncated}
                                                    {lastNote.length > noteLimit && <span className="note-cell-tooltip">{lastNote}</span>}
                                                </span>
                                            ) : <span style={{ color: 'var(--text-muted)', fontSize: 13 }}>—</span>}
                                        </td>
                                        <td style={{ color: 'var(--text-secondary)' }}>{formatSource(lead.source)}</td>
                                        <td style={{ color: 'var(--text-secondary)', fontSize: 13 }}>{new Date(lead.createdAt).toLocaleDateString()}</td>
                                        <td>
                                            <div style={{ display: 'flex', gap: 6 }}>
                                                <button className="btn btn-ghost btn-icon" onClick={() => navigate(`/leads/${lead._id}`)} title="View"><HiOutlineEye /></button>
                                                <button className="btn btn-ghost btn-icon" onClick={() => openEdit(lead)} title="Edit">✎</button>
                                                <button className="btn btn-danger btn-icon" onClick={() => handleDelete(lead._id)} title="Delete"><HiOutlineTrash /></button>
                                            </div>
                                        </td>
                                    </tr>
                                );
                            ...
                            <div className="modal-body">
                                ...
                                <div className="form-row">
                                    ...
                                    <div className="form-group">
                                        ...
                                        <select className="form-select" value={form.salesOwner} onChange={(e) => setForm({ ...form, salesOwner: e.target.value })}>
            ...
            <style>{`
                .note-cell-truncated { position: relative; }
                .note-cell-tooltip {
                    display: none; position: absolute; bottom: calc(100% + 8px); left: 0; z-index: 100;
                    background: var(--accent-secondary); color: #FFF7F3;
                    padding: 10px 14px; border-radius: 10px; font-size: 12px; font-weight: 500;
                    line-height: 1.5; max-width: 320px; min-width: 180px;
                    white-space: pre-wrap; word-break: break-word;
                    box-shadow: 0 8px 24px rgba(0,0,0,0.18); pointer-events: none;
                }
                .note-cell-truncated:hover .note-cell-tooltip { display: block; }
            `}</style>
        </div>
    );
}
